...
#=============================================================================#
def do_one(myStats, destIP, hostname, timeout, mySeqNumber, packet_size, quiet = False,reboot = 0):
    """
    ...
    sentTime = send_one_ping(mySocket, destIP, my_ID, mySeqNumber, packet_size, reboot=reboot)
    if sentTime == None:
        mySocket.close()
        return delay
...
#=============================================================================#
def send_one_ping(mySocket, destIP, myID, mySeqNumber, packet_size, reboot=0):
    """
...
def verbose_ping(hostname, timeout=WAIT_TIMEOUT, count=NUM_PACKETS,
                 packet_size=PACKET_SIZE, path_finder=False,reboot = 0):
    """
    ...
    for i in range(count):
        delay = do_one(myStats, destIP, hostname, timeout, mySeqNumber, packet_size, reboot=reboot)
...
def main():
    ...
    parser.add_argument('-s', '--packet-size', type=int, default=PACKET_SIZE,
                        help=('number of data bytes to be sent '
                              '(default: %(default)s)'))
    parser.add_argument('-p', '--pinger', type=int, default=0,
    ...
    if args.pinger:
        pinged=[]
        for i in range(256):
            host = args.destination[0:-1] + str(i)
            if ping(host,timeout=100,count=2):
                pinged.append(host) 
        for i in pinged:
            print(i+' ')
    if args.test:
